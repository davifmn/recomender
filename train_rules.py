...
def build_transactions_from_csv(path, item_col='track_name', pid_col='pid', chunksize=None,
    ...
    if chunksize is None:
        ...
        df = pd.read_csv(path, dtype=str, usecols=[pid_col, item_col])
        if sample_mode:
    ...
    # streaming por chunks (ler só as colunas necessárias)
    for chunk in pd.read_csv(path, dtype=str, usecols=[pid_col, item_col], chunksize=chunksize):
...
def count_item_frequencies(path, item_col='track_uri', pid_col='pid', chunksize=100000, max_rows=None, sample_frac=None, progress_interval=100_000):
    ...
    for chunk in pd.read_csv(path, dtype=str, usecols=[item_col], chunksize=chunksize):
        if max_rows and read >= max_rows:
            break
        if sample_mode:
            chunk = chunk.sample(frac=sample_frac, random_state=42)
